...
convergence stays my deliberate act."** Honored. The population of version verbs
was re-enumerated from reality for this cycle: every `skills/*/SKILL.md` under
all three family directories and the front-door plugin was listed (twenty-six
verb directories), and exactly two are version verbs, one per surface the Story
names — the planner family's `ok-version` for the session surface, and the
plumbline family's `version` for the project surface. Nine other files mention
the word "version" in prose only. Neither `ok-workspaces` nor the `/ok`
administrator carries one. Both verb files are pinned whole below, so a third
verb or a rewrite of either forces a re-audit.
...
installed plugin's."** Honored by the project-surface verb, whose Run block was
rewritten this cycle. It still runs the project's vendored binary if present
(and prints `project (vendored): none — /ok pins one to this project` if not),
and it still prints the carried payload's number beside it — but the payload
invocation is now guarded on the payload binary's executability, printing
`carried payload: none — the ok front door is not installed on this machine`
instead of dying when nothing is installed. That is strictly better for the
clause: the project's pinned number is now reported even on a machine with no
front door, which is the case where "what governs" and "what is installed"
differ most sharply. I confirmed it by running the verb from a clone of a
converged repository with `CLAUDE_PLUGIN_ROOT` unset: `project (vendored):
11.0.0` over `carried payload: none — …`, exit 0. The pinned copy is real rather
than notional: the plumbline converge core writes the binary into
`.ok-plumbline/bin/`, substituting the real suite version for the source's
`0.0.0-unvendored` placeholder, and the payload's own copy deliberately keeps
that placeholder — which the verb's closing paragraph names as expected.
...
body is still a read-only shell block printing two labelled lines and nothing
else. Its two new branches are presence tests on a file — `[ -x "$payload" ]` —
not version comparisons: no branch compares the two numbers, and neither branch
emits a judgement about which is newer. The prose that follows the block names
the payload's `0.0.0-unvendored` placeholder and the `none` line as *expected*,
which is description rather than verdict.
...
converges deliberately." The new closing sentence extends the same framing to
the not-installed case — "the project still lints at its own pinned version" —
again without computing anything.
...
running `plumbline version` against the freshly written file; I converged a
fresh fixture this cycle and read `11.0.0` back out of the vendored copy.

Carried forward from the previous cycle, since it still applies: the citation on
the estate hook's banner line does not pin the *number*. Release-mutable
metadata — including any `vX.Y.Z` on a line naming a suite family — is masked
before anchor comparison, so that `cite:` survives a version bump by design. The
in-step-ness of the estate is guarded by the harness's run-time assertion (this
project's stamp equals the manifest's version, recomputed on every run), not by
a citation.
...
the no-write / no-chain rule is explicit in both. The first limb is further from
true than it was: the project verb no longer needs an installed plugin at all to
report the governing number.
...
The story is annotated in the planner's `proofs.sh`, and both conjuncts are
...
with the carried manifest — the convergence signal. It additionally asserts the
verb still reads the conduct number from the active output style, and that on
this project the stamped artifact and the manifest agree. Re-run this cycle: all
four assertions pass, with the disagreement exhibited as `9.0.0-behind` against
the carried `v11.0.0`, and this project's own stamp confirmed equal to `11.0.0`.

**Proof coverage of the project surface.** Improved this cycle but still
partial. The plumbline family's new clone case runs the vendored `version`
verb's own Run block from a converged fixture with nothing installed and asserts
it exits clean without reaching for the payload — so the project-surface verb is
now executed by a deterministic case, where before it was only read. What no
case asserts is the *content* of its two lines against the fixture's stamp, so
the disagreement conjunct of the Proof field remains exercised only on the
session surface. Recorded as a partial; the Proof field asks for one exhibit of
disagreeing numbers, which the planner harness delivers.
...
verb echoes the vendored binary's number beside the payload's — now degrading to
`none` rather than failing when no front door is installed — and names the gap as
the converge signal without computing it; and the artifacts both verbs read are
genuinely stamped, as this checkout's materialized hook, at v11.0.0 and in step
with the manifest, shows. The story's proof exhibits its central claim from a
real converge run rather than asserting it as text, and it passes on the tree as
it stands.

Two non-determinative notes for a later reader. First, the Story clause's
...
citation anchors, so no citation in this audit detects a stale stamp — only the
harness does.
...
the conduct number stops coming from the active output style (the harness's grep
of "find the line that begins `Conduct version:`" breaks first); either verb
gains a verdict, a comparison, a write, or a chained action (the span pin on the
plumbline verb's whole Run block breaks on any new branch); the session-start
template stops carrying the version stamp, or the converge core stops
substituting it (the `cite:` on the template's `{{OK_PLANNER_VERSION}}` line
breaks — that token is not masked); the estate's stamped hook falls out of step
with the carried manifest (the harness's stamp-versus-manifest assertion turns
red); the plumbline converge stops vendoring and stamping the project binary,
making "pinned copies are real" false on the project surface; a third version
verb appears in any family (the two `cite-file:` pins are the population
source); or the planner's `proofs.sh` loses its `see-governing-versions` block
or its `@story:` annotation.
