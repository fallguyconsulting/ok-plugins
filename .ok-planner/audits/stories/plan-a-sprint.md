...
is interactive by construction (dialogue, walk, sign-off), and
self-sufficiency is stated as a property of the produced artifact and enforced
by what the artifact must contain.
...
the newest archived sprint carrying one, the window is
`git log <closed>..HEAD` plus the uncommitted tree, and an absent stamp is an
explicit ask rather than a guess.

**Acceptance 2 — "filtered to what bears on the corpus's commitments."**
Honored by a dedicated reviewer with a stated test (contradicts a commitment,
retires or bypasses a named mechanism, adds load-bearing structure the corpus
is silent on, edits `design/` directly) and a fixed tiebreak toward BEARING.
Only what it returns as BEARING is walked.
...
before the intake dialogue builds on them, with the deferral filed as an issue
so the question is held by the intake rather than by memory.

**Acceptance 4 — "the dialogue surfaces every tradeoff explicitly."** Honored,
as a prohibition: "surface every tradeoff explicitly — never resolve one
silently on the owner's behalf", with the proof-protection dialogue gate named
for the case where a delta implies an intent change.
...
The feature-work branch routes to draft first and consults the unruled issues
only afterward; the relevance pass runs over the draft; the walk is "one at a
time (never as a wall)"; and each issue is preceded by running the corpus
surfacer and reading what it prints in full, with an empty result treated as its
own signal.
...
recorded on the spot and the promotion carried into the sprint in final form.

**Acceptance 7 — "the draft passes compliance review."** Honored: the shared
compliance reviewer in draft mode, mechanical findings fixed in the draft,
judgment findings walked with the owner, re-dispatched until clean.
...
Honored. The template requires final-form artifact bodies ("No summarized or
partial deltas"), a flat unordered work-item list with staging explicitly
deferred to execution, and both boilerplate sections verbatim in every sprint.
The promotion timing is stated and justified: retirements during the walk,
`promoted` stamps at the terminal step after sign-off.

**Acceptance 9 — "The ceremony, its reviewers, and the compliance reviewer are
real components."** Honored: three dispatched reviewers with full prompts in
the file (out-of-band, relevance, compliance), plus the materialized
`surface-corpus` helper the walk invokes.

**Falsifier.** Each condition has a counterpart: self-sufficiency ("An
executing agent never reads an issue file to find out what a promoted issue
'really meant'"), the owner-only closure rule, the post-sign-off promotion
timing, the bearing-issue gate, and the out-of-band reconciliation phase.
...
are exercised. The first: the ceremony's template bakes all four governing
headings, the newest produced sprint carries the execution and completion
boilerplate, its deltas are final-form artifact bodies (asserted by finding a
`concept:`/`story:`/`decision:` frontmatter line inside a fenced delta block,
not merely the heading), and its work items are a flat list. The second: the
fold enumerates every issue file in `issues/` and `history/issues/`, requiring
each `promoted` file to name a sprint that exists on disk and each `retired`
file to carry non-empty text under `## Ruling`. The fold runs over this
project's real queue (44 walked, clean), over a seeded fixture, and — as a
negative control — over a fixture seeded with a promotion pointing at no
sprint, which it must reject. All assertions pass on the current tree.
...
surfacer, the compliance reviewer, and the post-sign-off promotion stamping
with its stated rationale. The story now carries an annotated proof covering
both conjuncts of its `Proof:` field, including the queue fold that guards the
ceremony's most owner-sensitive promise — the fold is exhibited failing as well
as passing, so it is a real check rather than a vacuous one.
...
in. That is deliberate and consistent with this story's Acceptance, but it
matters when reading `decision:relevance-scoped-queue-gate`, whose Choice
