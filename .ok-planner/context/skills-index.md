---
name: ok-planner
description: "ONLY activated by explicit slash command (/plan-sprint, /certify-work, /certify-all, /sketch, /audit, /prove, /verify-issues, /discover-design, /true-up, /ok-version). Never auto-triggered by conversation content."
---
...
| `ok-planner:audit` | User types `/audit`, or `/certify-all` runs it as a producer. Whole-corpus corpus review — a pure reporter that writes nothing: pass 1 checks every live artifact against the canonical rules (self-containment, current-state-only, story form incl. mandatory "so that", decision form incl. mandatory Proof); pass 2 checks proof coverage — presence **and cardinality** (each member a `Proof:` field enumerates must resolve in code, so "two implementations" cannot pass on one) — intent drift, and annotation integrity; pass 3 checks **cross-artifact consistency** (two decisions that contradict, a decision that forecloses a story's promised outcome), the drift no per-artifact check can see. Findings classify by intent, not file surface — a rules-determined, intent-preserving fix (corpus prose included) is mechanical, a redesign or material change to what the project commits to is judgment — but the class is advisory: every finding returns in-context to the caller, and the audit files nothing. Inside certification the review-fix loop drives them all to fixed-or-promoted; standalone, the human decides. |
...
| `ok-planner:certify-work` | User types `/certify-work`, or it is fired as the terminal step named by the sprint document's own execution boilerplate. The change-scoped "am I done?" gate — cost proportional to the change, not the corpus: aligns the work to its sprint (undershoot caught), runs `/prove` over the touched stories/decisions only, replaces whole-corpus audit with change-scoped corpus checks (compliance on touched artifacts, annotation integrity on changed files, coverage for touched artifacts, consistency of touched-vs-live), reviews the diff, and drives every finding to fixed-or-promoted through the shared no-discretion review-fix loop (fixer, then an architect that adversarially checks kickbacks — cap 3 fixer passes, scope never widens). Only architect-confirmed intent forks reach the issue intake, never asked live; `/verify-issues` makes them ruling-ready before the presentation. Presents outcomes and divergences, then offers the close-out — archive and commit on the owner's word, sprint left at its `sprints/` path until then. Default scope is the uncommitted tree; pass a git range (`/certify-work main..HEAD`) to certify committed work too. |
| `ok-planner:certify-all` | User types `/certify-all`. The whole-corpus certification gate: same machinery as `/certify-work` (shared review-fix loop, fixer and architect, reviewers, presentation, close-out offer) but at full scope — `/prove` executes and falsifier-exhibits every live proof, `/audit` sweeps every artifact, the code reviewer follows trails beyond the diff. Cost scales with the corpus; run it on a cadence — before a release, after several sprints, when a shared canonical rule changes, or when drift is suspected — not as the everyday close. |
| `ok-planner:verify-issues` | User types `/verify-issues`, or a certify gate invokes it after filing, or `/plan-sprint`/`/true-up` route a legacy `issues.jsonl` to it. Two stages: batched investigator subagents re-verify evidence, close corpus-answered issues, and repair rules-determined intent-preserving gaps (code- or corpus-side), returning factual briefs for the rest; then the main loop — never a subagent — rewrites each surviving issue as one from-the-top narrative (lede, causal mechanism, options with costs) and writes a marked generated or recommended ruling. Silence accepts a recommendation; the owner edits or empties to override. Never promotes or retires. |
...
- `.ok-planner/issues/` — the issue intake: one markdown file per design question requiring owner judgment. Filed by certification's architect / `/discover-design` / `/plan-sprint` / humans; verified ruling-ready by `/verify-issues`; closed only through `/plan-sprint`, by promotion into a sprint or retirement. Closed files live in `.ok-planner/history/issues/`.
...
Model follows the job: review, verification, investigation, and relevance dispatches run on sonnet; coding and fixing dispatches run on opus. Do not downgrade a coding job for savings, and do not upgrade a review by default. Subagent dispatch itself is disciplined — batched leaf agents that never recurse, open-ended agents guided to batch and to avoid fanning out (every agent has a 1M-token context) — per `skills/_shared/dispatch-discipline.md`.
