...
The project's public surface is captured in one prose document
committed to the estate — the **surface intent** per
...
Once the intent is landed, the extractor is dispatched. A
...
one entry per element found to the audit's own machinery record, the
surface extraction — kind discovered by
...
surface for that commit. The autonomous determinations follow — after
the documentation walk when `/document` composed the run
(`decision:documentation-walk-in-composed-audit`), immediately
otherwise.
...
The partition governs what the shipped vocabulary may name. The
story auditor drives only the public side, and a public document
names only the public side. A document type whose audience is
`developer` may name internal elements — scripts, entry points, the
layout of the tree — without moving them across the partition:
naming is not classification, and an element named in a developer
document is no more public for being named there.
...
- Per-kind committed member lists in the estate, one member per
  line, updated by the audit's opening walk. Retired
