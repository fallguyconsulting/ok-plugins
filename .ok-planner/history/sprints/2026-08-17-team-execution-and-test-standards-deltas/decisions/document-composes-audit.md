...
self-contained document per declared document type, at the vantage
the type's audience names: a public document names only the
extraction's public side in the shipped vocabulary, a developer
document may name internal elements without moving them across the
partition (`decision:documents-generated-per-type-and-placed`). The document
