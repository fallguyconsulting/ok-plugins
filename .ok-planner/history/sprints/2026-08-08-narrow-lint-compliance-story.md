...
One ruled issue, resolved as the owner directed: the story
`rules-compliance-report` promised a "read-only report of where my
project drifts from a skill family's declared rules" — vague on both
ends, and its "read-only" tripped a certification reviewer into
reading "writes no report" where the intent was audit-don't-fix. The
story is narrowed to the enumerable audited set (the codebase's
comments against its declared lint rules, its citations against the
corpus artifacts), states the audit changes nothing it examines, and
is renamed `lint-rules-compliance-report`. `story:corpus-audit`
stays as written — it already carries the artifact-side story.
...
### Retire story: rules-compliance-report

### New story: lint-rules-compliance-report
...
# Report of code drift from lint rules and corpus citations

## Story

As a project owner, I want a report of where my codebase drifts from its declared lint rules and where its citations of corpus artifacts fail to resolve — an audit that changes nothing it examines — grouped so I can tell mechanical fixes from structural questions, so that remediation happens at my direction rather than being applied silently.
```

## Work items

- Repoint every live reference to the retired slug at the renamed
  story (realizes `lint-rules-compliance-report`): the `@story:`
  annotations and the story-section registration in
  `plugins/ok/families/ok-plumbline/test/run.sh`, and the
  regenerated story catalog TOC line. The archive and the promoted
  issue file are records and are not touched.
