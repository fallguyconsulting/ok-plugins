...
Every documentation run re-derives and re-verifies every conclusion:
the assumption set is synthesized fresh, every story-way and
assumption is re-warranted, and no conclusion carries forward on any
validity test. Nothing tracks staleness and nothing invalidates
anything. The runnable instruments do carry: the experiment harness is
...
killed at every release and runnables are kept. Feeding the prior
