...
A finding is one defect surfaced by any of the suite's review passes — compliance review, coverage and drift checks, code review, test runs. Every finding is classified on one axis: mechanical (fixable without owner judgment — a forbidden section to strip, a stale index line, a dangling cross-reference with an obvious successor) or judgment (requires owner calibration — a boundary unstatable without naming a file, a story with no honest benefit clause, a decision whose rationale is a default rather than a tradeoff).
