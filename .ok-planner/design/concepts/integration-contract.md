...
The contract governs how plugins meet consumer projects; it does not govern any plugin's interior behavior. Its layers are realized by neighboring concepts: estate, cheatsheet, skill, true-up, materialized-artifact, stack-profile. The front door's own conduct is its consumer-side realization (see also: one-command-suite-upkeep under stories).

## Invariants

- Every integrable plugin exposes the lifecycle verb; plugins with rules to check also expose a read-only compliance verb.
