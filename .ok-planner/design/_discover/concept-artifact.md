...
No artifact kind carries a verification field today (the proof-mandate model this discovery observed is retired; verification is the implementation audit). Concepts carry invariants, and code cites concepts via `@concept:` annotations at load-bearing sites. The concepts catalog additionally has session-level standing: the generated `concepts.md` TOC is injected into every session by the session-start hook, with instructions to read the full file before using any term it names.
