...
The retired kinds map to the pre-4.0 skill suite, preserved only in git history: plans were written by `write-plan` and executed by the gate-driven `execute-plan` engine; `coverage` was a standing regression check over story acceptance; tensions were the predecessor of issue rows (same muddiness taxonomy, file-per-tension instead of JSONL events); review-notes belonged to the review skills. That suite also carried `brainstorm`, `verify`, `review-work`, `review-plan`, `refine-design`, `merge`, and `affirm` — none of which exists as a skill today.
