...
- `/ok-plumbline:ci`'s note says "The default templates clone plumbline from GitHub at lint time" while the same skill's preamble argues CI should run the committed vendored binary with "no install step" — the emitted templates and the vendoring doctrine pull in different directions (unverified which the binary currently emits).
