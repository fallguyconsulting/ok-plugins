...
This is a reversal of the plugin's own pre-4.0 architecture, preserved in `design-notes/`: the flip-gated execution engine (2026-06-05) was a full workflow engine — baseline snapshots, gate pre-flight classification, artifact probes, gate repair with flip-validation, an escalation taxonomy, cap accounting, a strategist rung. The 4.0 rework (commit 1d4af77 "corpus-spec + planning-ceremony rework") deleted the engine skills (write-plan, execute-plan, execute-plan-in-worktree, brainstorm, verify, review-work, review-plan, coverage, refine-design, affirm, merge, sketch survived) and moved the verification burden into the corpus itself (proofs with exhibited falsifiers) plus terminal gates (`/prove`, `/audit`, `/certify`). What survived the reversal: the flip principle (as falsifier exhibition), the completeness contract (as sprint boilerplate step 5 and ok-conduct's floor rule), asymmetric divergence (certify's "An undershoot must never appear here — it was fixed, not reported"), and the never-destroy-work rule.
...
- `plugins/ok-planner/CLAUDE.md` plugin purpose; `design-notes/2026-06-05-flip-gated-execution.md` and `2026-06-06-completeness-contract.md` (the predecessor architecture, marked "design, pre-implementation" and never updated after the reversal).

## Adjacent topics

- `sprint`, `completion-contract`, `certify-gate`, `proof-and-falsifier` (the flip principle's survivor), `ok-conduct` (run-unsupervised + completeness floor), `pre-4-0-kinds`.
