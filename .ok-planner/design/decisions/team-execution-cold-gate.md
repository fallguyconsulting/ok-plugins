...
the code, applies the stage's corpus deltas, and records its calls and forks as items in the run's `divergences`
...
its judgment passes cover — cuts those files into areas, files each
area into the tracker, files one pass task per pass forked from its
own task, closes its own task, and forks one agent per pass task in
one message: one enumeration pass over the whole change, where the
heading reduces to listing a population and grepping the tree for
each member; a correctness pass per area; and, with a sprint in
scope, the sprint-alignment pass. An area is a package the change
touched, a changed definition pulls its changed callers in, a corpus
delta rides with the code it governs, and no file or line budget
splits an area or merges two. Every fork inherits the root's reading
as a cached prefix, claims the pass task its prompt names, files its
own findings under that task, enumerates its population before it
judges, and closes the task with the population it checked. The
tracker issues a pass task to nobody while a fork may hold it; a
pass task a fork left open is reissued by the drain to a fresh agent
of the profile, which reads its population cold. Each family's
mechanical producers run beside the root, each a read-only task. The tracker's triage re-keys
...
file and changes no runtime behavior; a round
...
one defect class across its sites, one surface's files — and files one fixer task per batch, its one judgment
...
reach, and the sibling members of a defect class. A defect a
...
the judgment passes cover only the files the previous round's fixer
and architect staged, cut by area, while the enumeration pass covers
the whole change. A fixed finding is verified at
triage when no open finding names its site, because a pass read the
...
the root and its code-review passes stay blind to the report, so an
unrecorded divergence surfaces as a fresh finding. The alignment pass
reads the report after its reading, hands each divergence to the veto
...
population and greps the tree for each member is enumeration:
...
contract is judgment. Every pass rides the stronger profile, per
`decision:subagent-model-follows-job`: an earlier trial showed the
cheaper profile reporting nothing where the stronger one found four
defects in the same files, a later run's stronger-profile round filed
load-bearing findings the cheaper rounds after it did not have the
chance to find, and two reviewers over one staged change returned
seven findings on the cheaper profile and fourteen on the stronger,
the cheaper one clearing a lint check the stronger one showed
failing open. The stronger profile's tasks cost more than half again,
and the review root pays that once per reading, since the forks
share it. The review root exists because that fixed cost is the
...
what the judgment passes read, and the root reads the rest once.
...
and the root's recovery reading cost more than the findings it
...
leaves the pass that receives the callers paying to learn the
...
— so the fixer's checks passed and a reader found each one a round
later.
...
two findings in two files share one cause.
...
reads the staged files in its judgment passes and the whole change in
its enumeration pass. One run under a staged-only
shape closed clean with five orphan symbols in three files no fixer
touched; orphan symbols are what the enumeration pass lists, and it
covers the whole change every round. In the measured run, seventeen of
...
each judgment pass was given, and every covered file is in exactly
...
- Every pass on the cheaper profile — three passes together cost
  under half of one pass on the stronger one, and the measured
  comparisons show it clearing defects the stronger profile finds.
- The judgment passes on the stronger profile and the enumeration
  pass on the cheaper — the enumeration pass judges too: it decides
  whether a remaining reference is stale and whether a test-path
  write is a test.
...
- Forks that file findings under the root's open task and return a
...
- Every round's judgment passes over the whole change — the measured
