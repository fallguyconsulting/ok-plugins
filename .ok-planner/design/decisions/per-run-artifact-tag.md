...
Verification needs two properties: parallel runs must not collide on an artifact, and a run must verify the artifact it just built. A tag unique per run gives both directly. Content addressing gave both only as a side effect of a stronger claim, that identical trees produce identical tags on every machine. Beyond the two properties, that claim gave one thing more: reuse of an artifact across runs, which is a dev-loop convenience.
