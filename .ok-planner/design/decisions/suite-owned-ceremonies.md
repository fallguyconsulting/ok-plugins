...
conventional contribution the ceremony drives — the counterpart, on the
ceremony axis, of the administration files the front door drives.
...
front door does by driving conventional administration files.
Ceremonies were the axis that had not been hoisted, and the cost showed
in the collision
...
contributions is what stops every project from vendoring every family's
...
- Ceremonies stay family-owned and each family implements its own —
  no new conventional contribution, and the owner runs every ceremony
  once per family and reconciles the results.
...
- One hoisted body carrying every family's instructions inline —
  no per-family contribution to design, paid for in context on every
  read in every project, whether or not the family is present.
