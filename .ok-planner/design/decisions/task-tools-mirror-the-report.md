...
task per stage, mirroring the completion report's staged list, marked
done as each stage lands. The session keeps that list, so the
...
because it survives the session and is what a replacement builder
reads; the checklist survives nothing. The setting is a consent
...
- The task list as the staged list itself — evaporates with the
  session, and a replacement builder has nothing to read.
