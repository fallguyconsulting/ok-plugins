...
Materialized by ok-planner v13.0.0. Suite-owned
...
- **`stories/`** — durable user expectations, each one agile-style
  ...
  accomplish the benefit. Verification is the implementation
  audit's: where a
  story is implemented in code, the audit cites the test or tests in
  the project's ordinary suites that exercise it end-to-end; where
  it is realized in prose, the audit cites the relevant prose.
...
Annotations carry exactly one job — this navigation — and play no
part in certification scope or audit invalidation: what a change puts in
...
implemented the work, never hand-edited. Each audit is a plain
pass/fail with a terse paragraph or a few bullets of reasons — the
auditor believes the story is enacted (for a coded feature:
complete and exercised end-to-end by the cited tests) or the
decision's choice is respected — followed by its citations: code
inside tests, or tightly-scoped prose, each one meaning "reconsider
this audit if this changes." No history, no hypotheticals. A determination attaches
...
structural identities (`path#declaration-chain`) and content hashes,
edges from syntactic reference. `.ok-planner/bin/source-graph build`
regenerates it wholesale; identical trees yield byte-identical
graphs, so regenerating is always safe, and `check` flags drift. The
graph contains no judgment: audits point into it from outside, by
`cite-node:` lines (with `cite:` / `cite-span:` / `cite-file:` as
finer or pre-graph anchor forms), generated by the vendored helper
`.ok-planner/bin/audit-check cite-node <identity>`.

What forces a re-audit is **two layers, never annotations**:
mechanically, `audit-check` flags any audit whose design artifact
hash moved, whose cited node no longer resolves, or whose cited
hashes or anchors changed (`--list-stale` prints the set; a pure
move — a vanished identity whose recorded hash matches exactly one
node in the fresh graph — is re-pointed in place by `audit-check
repoint` first, never re-audited); by
judgment, certification's change inspector reads the diff itself and
nominates audits whose claimed territory contains changed code no
citation caught. Nominations land as provisional entries in the
committed inspection registry; the auditor adjudicates each —
promoted into a citation or dismissed with a reason — and recorded
adjudications bind later runs unless the cited reality moves. Audit
files carry no notes and no history: an audit is a brief
confirmation plus citations, describing only the project as it
stands at audit time. Certification also dispositions every
hunk of a certified change (mechanical / adjudicated / residue — the
...
suites, the implementation audits over the two-layer re-audit set
...
coverage: implemented means implemented and covered, tests cited
