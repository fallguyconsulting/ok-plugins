...
**Guard clause.** This file governs the run only where its
measurement front can stand: if ensuring a current audit runs
`/audit` and that audit's surface partition is not settled, the goal
was set too early — say exactly that and stop. Never settle a
partition question alone; the walk is the owner's.
