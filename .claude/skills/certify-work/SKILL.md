...
4. **Producers** — assemble the run's producers: the two this body always runs, plus every producer each present contribution declares under its `Producers` phase. Producers are stateless reporters: they never file issues and never fix. The gate is cold and is the only review the work gets: the build files no review task, no producer reads the build's findings, and among the producers only the sprint-alignment judge reads the completion report. A command producer is an `exec` task; an agent producer is a task under its profile.
   ...
   - **Code review, scoped to the diff, planned per round, in passes.** The `review-planner` prompt is `{{CERTIFY-REVIEW-PLANNER-PROMPT}}` and the `gate-review` prompt is `{{CERTIFY-CODE-REVIEW-PROMPT}}`, both from `../_shared/certification-core.md`. Every round the gate files the planner first, which cuts the change's files into batches it files into the tracker, then the passes in the review prompt's table under the profile each row names: the two enumeration passes once each over the whole change, the two judgment passes once per batch with the batch as the task's files. Both prompts take `[REVIEW SCOPE]`: the planner the first two paragraphs below, which say what the change is and what is out of it; the passes all three. Filled as:
...
<!-- Materialized by ok v20.2.0 — suite-owned; overwritten on converge; do not hand-edit. -->
