...
5. **Present.** Compose and deliver `{{CERTIFY-PRESENTATION}}` from `../_shared/certification-core.md`. The per-producer "Findings fixed" lines for this gate: alignment, prove (touched), implementation audit (re-audit set, with flipped determinations counted), corpus checks (change-scoped), code review.
