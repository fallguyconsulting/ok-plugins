...
`/prove` produces work items for an **agent**, not a human: its findings return in-context as a structured report the executing agent consumes with its own triage. It **never writes to the issue intake (`.ok-planner/issues/`)** — filing for the human belongs to `/ok-planner-audit`. If a prove finding turns out to need owner judgment (an intent question, not a broken proof), the escalation path is the next `/ok-planner-audit` catching the underlying corpus problem.

Read `{{PROOF-PROTECTION-RULE}}` in `../_shared/artifact-definitions.md` before starting — it defines what a proof is, the annotation link, and non-vacuity.
