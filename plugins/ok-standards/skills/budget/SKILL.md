...
```
/ok-standards:budget          # report current vs baseline (CI-suitable)
/ok-standards:budget save     # record current count as the new baseline
```
...
case "$action" in
  ...
  check)
    ...
    ;;
  *)
    echo "usage: /ok-standards:budget [save|check]" >&2
    exit 1
    ;;
esac
```
...
- **Below baseline:** propose committing the new lower baseline with `/ok-standards:budget save`. The ratchet is one-way; once committed, future increases fail CI.
