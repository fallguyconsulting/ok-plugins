...
- The canonical cheatsheet (`docs/plumbline-cheatsheet.md`) materialized into consuming projects by `/ok-standards:affirm`.
...
If `docs/plumbline-cheatsheet.md` is among the changed files, mention in the final report that consuming projects will need to re-run `/ok-standards:affirm` to pick up the new cheatsheet. This is informational only — never blocks the release.

### 5. Apply the bump

The version string is duplicated in two places — both must move together so `/ok-standards:version` keeps reporting the truth:
