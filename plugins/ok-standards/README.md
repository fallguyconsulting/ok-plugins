...
This writes `.claude/rules/plumbline-cheatsheet.md` into the current project — the rules file every Claude Code session in the project will read. Commit it. Re-run `/ok-standards:affirm` after a plugin upgrade to pick up the latest canonical cheatsheet.
...
- [docs/plumbline-cheatsheet.md](docs/plumbline-cheatsheet.md) — the compact form materialized into consuming projects by `/ok-standards:affirm`.
- [docs/plumbline-porting-guide.md](docs/plumbline-porting-guide.md) — the migration arc for adopting Plumbline on an existing codebase. Phase-by-phase, tool sequencing, decision points, plan template. Consume directly or via `/ok-standards:port` (emits a project-specific plan with backlog numbers filled in).
...
Each subcommand is also wrapped as a Claude Code skill: `/ok-standards:patterns`, `/ok-standards:budget`, and so on.
...
`citations` is the only way to declare project-specific allowed comment forms; each entry must pair a tag with a structural resolution rule. `/ok-standards:starter` produces a project-shaped config (including the ok-planner citation entries above when it detects `.ok-planner/`).
