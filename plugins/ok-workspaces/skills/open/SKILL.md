...
Create an isolated workspace for one job: its own worktree, own branch, namespaced runtime. Requires a committed profile (`.ok-workspaces/config.json`) — if absent, run `ok-workspaces:true-up` first and stop at its proposal step.

**Worktrees live inside the project by default** — `.ok-workspaces/worktrees/<job>`, gitignored by true-up — so a job's checkout never escapes the project root and cannot be stranded next to it. A project that wants them elsewhere (a sibling directory, a scratch volume) declares that in its profile's `worktrees.dirPrefix`; the committed profile always wins.
...
2. **Create the worktree.** From the main checkout's repo root:
   ...
   If `<dirPrefix>` is inside the repo (the default), `.ok-workspaces/.gitignore` must already cover it — true-up writes that file. If it is missing, run `ok-workspaces:true-up` before creating the worktree rather than leaving a checkout that `git status` offers to commit.
