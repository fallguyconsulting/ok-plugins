...
Every auditor is a task in one run of the task tracker at `.ok-planner/bin/tasks`, and so is the judge. The ceremony files the tasks. The `execute-tasks` loop drains the run and dispatches them. The reason is the prompt cache. Every agent of one profile starts from the profile's system prompt and one message that differs only in the task id on its last line, so the project context is one cached prefix per profile for the whole run; the task's brief arrives as a tool result after the prefix. The planner contribution owns the run's shape — the run file, the prompt files, the profiles, the brief format, the pools. This spine says how the run is driven:
...
- Does not build the project. The measurement instrument does execute the released product — only through elements the extraction records public, per each estate's protocol.
