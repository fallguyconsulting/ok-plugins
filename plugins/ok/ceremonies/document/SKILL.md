...
took at this release, every element the surface ruling classifies
...
**This ceremony measures nothing.** The audit rules the surface
partition, determines story support from the user's side (experiments
driven through the ruled public surface on the maintained
...
supported stories are the delivery criterion, the ruling's public side
is the catalog domain, the story and assumption determinations become
the assessments, and the assumption dispositions become the trap
registry. Composition, never absorption — one canonical audit body
exists, and the two ceremonies cannot drift apart on what an audit is.
...
- **The verification layer** — the surface ruling, the audit's
...
2. **Ensure a current audit.** The audit is current for this release
   ...
   records, and its surface ruling.
...
stamped commit — taken by the audit, on the maintained experiments.
...
- Does not edit the design corpus, the surface declaration, the
  guidance, the ruling, any audit record, or any code.
...
- Does not ask the owner anything mid-run. Its measurement front — the
  audit — holds the one interactive moment; this run constructs,
