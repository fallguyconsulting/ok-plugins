...
The front door's administration converges the family into the project: it writes `.claude/rules/plumbline-cheatsheet.md` — the rules file every Claude Code session in the project will read (commit it) — vendors the lint binary, the skills, the writing, testing, and events standards (under `.ok-plumbline/docs/`), the pre hook (`.ok-plumbline/hooks/pre-write.js`), the edit hook (`.ok-plumbline/hooks/post-edit.js`), and the review hook (`.ok-plumbline/hooks/stop-review.js`), and, on your consent, wires four entries into `.claude/settings.json` — `PreToolUse` and `PostToolUse` on every tool, `Stop` and `SubagentStop` — on one consent. From then on, for every agent — the main session and dispatched subagents alike — the pre hook stamps a start marker for each Bash call so the post hook can find the files that call wrote; `plumbline` runs on every Edit/Write and blocks (exit 2) when violations are found, so the agent fixes them in the same turn; the post hook lists any tool call that wrote prose to a file under the project root, a Bash heredoc's target file included, and skips a scratch file outside the root; a commit message changes no file, so it is not a written source; while that list stands, every later tool call carries a silent reminder to review every sentence written in those files against the standard at the end of the work and then clear the list with `echo plumbline:prose-reviewed`; and if an agent stops with the list still standing, the review hook continues the turn once as non-error feedback with the same instruction. Re-run `/ok` after a plugin upgrade to converge to the latest version.
