...
The front door's administration converges the family into the project: it writes `.claude/rules/plumbline-cheatsheet.md` — the rules file every Claude Code session in the project will read (commit it) — vendors the lint binary, the skills, the writing, testing, and events standards (under `.ok-plumbline/docs/`), the pre hook (`.ok-plumbline/hooks/pre-write.js`), the edit hook (`.ok-plumbline/hooks/post-edit.js`), the review hook (`.ok-plumbline/hooks/stop-review.js`), and the instructions script it names (`.ok-plumbline/hooks/stop-instructions.js`), and, on your consent, wires four entries into `.claude/settings.json` — `PreToolUse` and `PostToolUse` on every tool, `Stop` and `SubagentStop` — on one consent. From then on, for every agent — the main session and dispatched subagents alike — the pre hook stamps the turn's start once, on its first tool call; `plumbline` runs on every Edit/Write and blocks (exit 2) when violations are found, so the agent fixes them in the same turn; the post hook lists any Edit or Write that put prose in a file under the project root and skips a scratch file outside the root; at the stop, the review hook walks the tree once for every file modified since the turn's stamp, a Bash heredoc's target file included, staged or not, and adds those holding prose to the list; a commit message changes no file, so it is not a written source; and when an agent stops with that list standing, the review hook continues the turn once as non-error feedback with one line — run the instructions script (`.ok-plumbline/hooks/stop-instructions.js`) and follow what it returns — and the script returns the instruction: review every sentence written in those files against the standard, rewrite what fails, then stop. Re-run `/ok` after a plugin upgrade to converge to the latest version.
