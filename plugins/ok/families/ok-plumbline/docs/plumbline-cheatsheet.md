...
- A `PostToolUse` hook, on every tool call, runs the lint over the file an Edit/Write touched — violations block (exit 2) so the agent fixes them in the same turn — and detects prose the call wrote under the project root: file content, `new_string`, a heredoc's target, a commit message. Text written outside the project root — a scratch file under a temp directory — is not counted. Detection flags the agent's turn; it blocks nothing.
