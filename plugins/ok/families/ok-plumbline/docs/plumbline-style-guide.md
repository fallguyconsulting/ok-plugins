...
Tooling syntax is not prose. The lint exempts comments whose significant lines match:
...
**A directive exempts its own line, never what is written under it.** The
test is per line, so prose does not become permissible by sitting beneath
a license header, a suppression, or a build tag: the first line of a
comment block that is neither a directive nor a citation is the
violation, reported at that line. The one narrow continuation allowed is
license and generated-file boilerplate under the notice that opens it —
the standard "you may not use this file…" body of an Apache or GPL
header — because that text is the directive, not commentary on the code.
A wholly blank comment line (`//`, `#`) carries no content and is neither
exempt nor a violation; it is residue worth deleting on sight.
