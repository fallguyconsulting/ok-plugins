---
name: events
description: "ONLY activated by explicit /ok-plumbline:events slash command. Never auto-triggered by conversation content. Inventory every structured event kind the code emits — its emitting sites and the tests that wait on it — plus format violations, orphans, and the pruning list. Read-only — reports, fixes nothing, files nothing."
---
