...
| `/execute-tasks` | Drains the task tracker's selected run: calls next, starts as many agents of each profile as the count it prints, up to the concurrency cap, every agent under one identical message that names no task, runs each exec task itself, stamps usage on each task as its agent returns, and stops at done, at waiting, or at a task nobody claimed. Files nothing and judges nothing. |
