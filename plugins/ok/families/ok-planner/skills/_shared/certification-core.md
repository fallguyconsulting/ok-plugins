...
Every agent this file defines is a **task** in the task tracker at `.ok-planner/bin/tasks`, dispatched by the `execute-tasks` drain under a vendored profile. The reason is the prompt cache: every agent of one profile starts from the profile's system prompt and one fixed message that names no task, and `tasks claim --agent <profile>` takes the oldest issued task for the profile and hands it its id, its prompt, its brief, and the pool items it consumes as a tool result, after the cached prefix; only a fork claims by id. No agent stands across rounds. Each task is one bounded piece of work; what it found or did goes into the run's pools, and the next task reads the pools, never a predecessor's context.
