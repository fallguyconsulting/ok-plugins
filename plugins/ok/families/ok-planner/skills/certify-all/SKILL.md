...
4. **Verify the promoted issues** — only if the architect promoted any. Invoke `ok-planner:verify-issues`; it makes everything promoted this run ruling-ready per its own process (closing what the corpus answers, narrating the rest to a ruling the owner accepts or overrides). Zero promotions → skip, silently.
