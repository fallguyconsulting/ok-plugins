...
The ceremony files it; this family adds one check: a judgment pass opens every file a sprint's deltas affect under `.ok-planner/design/` in its area and verifies each delta landed correctly. Every delta is due here. The gate reviews the finished work; the build runs no review of its own.
