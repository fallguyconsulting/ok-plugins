...
`ok-planner` is the specification for an opinionated documentation corpus — concepts, stories (agile-style non-prescriptions of user need with a mandatory "so that" clause), decisions (technical choices) — both kinds verified by the periodic implementation audit, which answers per artifact whether the codebase supports it at a named commit — plus the planning ceremony (`/plan-sprint`) that maintains it, the pre-commitment sketch verb (`/sketch`), the corpus verb (`/audit`), the issue verifier (`/verify-issues`), the change-scoped certification gate (`/certify-work`), the periodic audit (`/verify-corpus`), and the bootstrap (`/discover-design`). Execution works directly from the sprint document: `/plan-sprint` bakes a fixed "How to execute this sprint" section into every sprint it produces, so the sprint can be picked up inline, handed to the native `goal` mechanism, or dispatched to an orchestrator that does its own planning — every executor works from the same brief. `/certify-work` discharges the completion contract at the change's scope with the review cycles and a presentation, and audits nothing — the corpus's claims are `/verify-corpus`'s question, on the owner's cadence. The shared certification machinery (fix loop, fixer, reviewer, presentation) lives once in `skills/_shared/`, as do the auditor and judge prompts. There is still **no plan artifact**: a sprint is never rewritten into a plan; staging happens at execution time in the executor's own working state.
...
skills/_shared/                   # Canonical artifact definitions, certification core, implementation auditor and audit judge, dispatch discipline, shared reviewer prompt (transclusion sources)
scripts/audit-check               # Audit-corpus shape-and-invariant validator (python, exit 0/2/1): coverage, frontmatter shape, one-paragraph brevity, and the rule that a non-supported determination names its issue; materialized to consumer .ok-planner/bin/audit-check
...
test/run.sh                       # audit-check test harness (fixtures for shape, brevity, coverage, and the issue-link invariant)
...
**Verification is the audit's.** Whether the codebase supports a story's or decision's claims is the **implementation audit**'s determination (`supported` | `unsupported` | `unclear`), written only by the periodic `/verify-corpus` run and recorded under the consumer's `.ok-planner/audits/`. An audit is one sentence to one paragraph about a named commit — every universal reported as a count plus the population it came from, and no citations, hashes, or line numbers anywhere. Nothing computes staleness: whether an audit still holds is how far `HEAD` has moved since the commit it names. `audit-check` enforces shape, brevity, coverage, and the one invariant that a non-supported determination names its issue.
...
Skills do not chain into a pipeline. `/plan-sprint` is terminal at the approved sprint; `/audit` is invoked by the user or by whoever executes a sprint's completion contract. Ceremonies ensure their own layout with a `mkdir -p`; estate convergence is the front door's administration, never a ceremony's.
