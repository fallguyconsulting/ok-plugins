...
browser/                          # The corpus view's Svelte frontend: src/ is the source, dist/ is the release-built bundle carried as payload and placed at .ok-planner/browser/ by converge
scripts/corpus-view               # The corpus view's stdlib-only service (python): serves the built page over loopback and resolves every citation through the project's own audit-check; materialized to consumer .ok-planner/bin/corpus-view
scripts/browse                    # The corpus view's up/down helper (python): starts the sibling corpus-view on any free port, records pid/port in the estate's gitignored run/, opens the system browser; materialized to consumer .ok-planner/bin/browse
scripts/ok-planner-gitignore      # The estate's own ignore file; keeps generated estate content (the fetched view build, the browse script's run state) out of the repository and out of the source graph
...
test/run.sh                       # audit-check test harness (fixtures for staleness, citations, violations)
...
**Verification is the audit's.** Whether an implementation genuinely satisfies a story's or decision's claims is the **implementation audit**'s adversarial determination (`satisfied` | `violated`), written only by certification's auditor, recorded under the consumer's `.ok-planner/audits/`. The audit identifies where and how the artifact is implemented: it cites the end-to-end tests in the project's ordinary suites for whatever is implemented in code, and the relevant prose — narrowly — for the rest; `audit-check` computes staleness and the re-audit set.

The issue intake (`.ok-planner/issues/` in consumer projects) is one markdown file per issue, timestamped so filenames sort chronologically. Section ownership is strict: filers (certification's architect, `/discover-design`, `/plan-sprint`, humans) write Problem and Candidates; `/verify-issues` writes the from-the-top narrative (closing corpus-answered issues with a citation); the owner — and only the owner — writes unmarked `## Ruling` text. Files close through `/plan-sprint` (promoted into a sprint or retired) and move to `history/issues/` — retirements immediately, promotions when a certify gate archives their sprint. A legacy `issues.jsonl` is converted by `/verify-issues` and archived to `history/issues.jsonl`, its rows never rewritten.
...
- No Node tooling **at runtime**: skills are markdown, hooks are bash, support scripts are bash or python, and nothing a consumer project runs needs node. The one exception is build-time and stays in this repo — `browser/` is a Svelte app compiled once per release by `/release`, whose output ships as payload; its `node_modules/` is never committed and never vendored.
