...
scripts/proof-timings             # Deterministic proof-run cost recorder (python): times each proof, merges the durable record at .ok-planner/proof-timings.json; materialized to consumer .ok-planner/bin/proof-timings
...
test/proofs.sh                    # The family's story proofs, one section per story, each reporting what proving it cost
```
